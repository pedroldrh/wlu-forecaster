...
export function ConsensusChart({ data, userProbability = null }: ConsensusChartProps) {
  ...
  // Two-phase mount: first render the container, then init chart after paint
  useEffect(() => {
    setMounted(true);
  }, []);
  ...
  useEffect(() => {
    ...
    // Wait for next animation frame so container has real dimensions
    let cancelled = false;
    const raf = requestAnimationFrame(() => {
      ...
      // If width is still 0, try again after a longer delay
      if (width === 0) {
        const retryTimer = setTimeout(() => {
          if (!cancelled && containerRef.current) {
            createChartInstance(containerRef.current);
          }
        }, 300);
    ...
    function createChartInstance(container: HTMLDivElement) {
  ...
  }, [mounted, data, latestValue, userProbability]);
    ...
    <Card className="overflow-hidden border-border/60">
      <CardContent className="p-0">
        ...
        {data.length > 0 ? (
          ...
          <div className="flex items-center justify-center h-[240px] -mt-4 text-muted-foreground text-sm">
            No forecasts yet
          </div>
        )}
      </CardContent>
